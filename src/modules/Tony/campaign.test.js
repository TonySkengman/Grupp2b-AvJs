...
test("Cart räknar ut rätt subtotal", () => {
  const cart = new Cart([{ productId: "1", unitPrice: 100, quantity: 2, category: "böcker" }]);
  assert.equal(cart.getSubtotal(), 200);
});

test("Cart kastar InvalidCartError om varukorgen är tom", () => {
  // Oanropad funktion (() => new Cart([])) - assert.throws kör den själv
...
test("PriceCalculator staplar rabatter i rätt ordning", () => {
  ...
  // 200 - 50 (threshold, körs först) = 150, sen 10% av 150 = 15 -> 135 kvar.
  // Bevisar att stapelordningen faktiskt respekteras, inte listans ordning.
